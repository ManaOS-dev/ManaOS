...
pub fn load_user_program(
    ...
    crate::log_info!(
        "elf",
        "User ELF demo loaded: path={} entry={:#x} heap_start={:#x} heap_start_typed=true",
        source_path,
...
pub fn validate_user_program_image(image: &[u8], source_path: &str) -> bool {
    match load_user_elf_metadata(image) {
        Ok(loaded) => {
            crate::log_info!(
                "elf",
                "User ELF image validated: path={} entry={:#x} heap_start={:#x} heap_start_typed=true",
                source_path,
                loaded.entry_point().as_u64(),
                loaded.heap_start().as_u64()
            );
...
impl LoadSegmentRange {
    fn from_program_header(program_header: ProgramHeader) -> Result<Self, LoadError> {
        ...
        let memory_range = UserVirtualRange::new(segment_start, memory_size)
            .ok_or(LoadError::SegmentAddressOutOfRange)?;
        let last_byte_address = memory_range
            .end_exclusive()
            .checked_sub(1)
    ...
    fn start(self) -> UserVirtualAddress {
...
fn load_user_elf(
    ...
    Ok(LoadedElf {
        entry_point,
        heap_start: heap_start.ok_or(LoadError::NoLoadSegments)?.as_address(),
    })
}
...
fn map_load_segment(
    ...
    loop {
        ...
        if page_start == segment_range.last_page {
            break;
        }
