...
//! - [`spawn`] - Spawn a child user program
//! - [`spawn_with_vectors`] - Spawn a child user program with explicit entry vectors
...
pub use api::{
    ...
    read, spawn, spawn_with_vectors, waitpid, write, AT_FDCWD, ERROR_ARGUMENT_LIST_TOO_LONG,
    ERROR_BAD_ADDRESS, ERROR_BAD_FILE_DESCRIPTOR, ERROR_FILE_EXISTS, ERROR_INVALID_ARGUMENT,
    ERROR_IS_DIRECTORY, ERROR_NOT_DIRECTORY, ERROR_NOT_FOUND, ERROR_NOT_IMPLEMENTED,
    ERROR_NO_CHILD, ERROR_OUT_OF_MEMORY, ERROR_RANGE, ERROR_TRY_AGAIN, FILE_TYPE_DEVICE,
    FILE_TYPE_DIRECTORY, FILE_TYPE_REGULAR, MAP_ANONYMOUS, MAP_FIXED, MAP_FIXED_NOREPLACE,
    MAP_PRIVATE, OPEN_CLOSE_ON_EXEC, OPEN_READ_ONLY, PROT_EXEC, PROT_READ, PROT_WRITE, SEEK_CUR,
    SEEK_END, SEEK_SET, SYS_BRK, SYS_CHDIR, SYS_CLOSE, SYS_EXECVE, SYS_EXIT, SYS_EXIT_GROUP,
    SYS_FSTAT, SYS_GETCWD, SYS_GETDENTS64, SYS_GETPID, SYS_GETPPID, SYS_LSEEK, SYS_MMAP,
    SYS_MUNMAP, SYS_NANOSLEEP, SYS_OPEN, SYS_OPENAT, SYS_READ, SYS_SPAWN, SYS_WAITPID, SYS_WRITE,
    WAIT_ANY, WNOHANG,
};
pub use contract::{
