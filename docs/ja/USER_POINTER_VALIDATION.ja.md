...
- NUL-terminated path string は syscall-specific maximum length 付きの
  `copy_cstr_from_user` を使います。この helper は最初の NUL terminator まで byte ごとに検証するため、
  mapped user page の終端付近にある短い string でも、terminator 自体が readable なら受け付けます。
- NUL-terminated pointer array は、指している string を読む前に各 pointer slot を
  `copy_from_user` でコピーします。
- non-zero syscall pointer argument は raw `u64` ABI value から
  `UserReadableRange`、`UserWritableRange`、または `UserCString` の
  copy-direction constructor で分類してから copy helper に渡します。
- page-table permission probe はその copy-direction wrapper を受け取ります。raw `usize`
  pointer は final kernel slice を作る境界、または分類済み user byte を読む境界だけで使います。
- permission probe は page-table entry を translate する前に、first / last page walk position を
  `UserVirtualRange` から `UserPageStart` として導出します。
- user address-space permission self-check は代表 kernel probe を `VirtAddr`、代表 user probe を
  `UserVirtualAddress` として保持し、その後 single-byte readable / writable probe range を作ります。
- NUL-terminated path argument は、`copy_cstr_from_user` が terminator を探す前に
  syscall constructor で `UserCString` に分類します。
...
| `execve(path, argv, envp)` | `path`, `argv`, `envp` | user to kernel | `copy_cstr_from_user`, `copy_from_user` | path は `MAX_USER_STRING_LENGTH` で capped されます。`argv == NULL` と `envp == NULL` は empty vector として扱います。argument と environment vector はそれぞれ 8 entries、NUL terminator を含む copied string bytes は合計 4096 bytes で capped されます。invalid pointer は `-EFAULT`、limit overflow は `-E2BIG` を返します。valid ELF image は current user image を置換し、old instruction pointer へ戻りません。 |
| `spawn(path, argv, envp)` | `path`, `argv`, `envp` | user to kernel | `copy_cstr_from_user`, `copy_from_user` | path は `MAX_USER_STRING_LENGTH` で capped されます。`argv == NULL` または empty vector の場合、resolved path を default `argv[0]` として使います。`envp == NULL` は empty vector として扱います。argument と environment vector は `execve` と同じ 8-entry / 4096-byte copied string limit を使います。invalid pointer は `-EFAULT`、limit overflow は `-E2BIG` を返します。 |
| `waitpid(pid, status, options)` | optional `status` | kernel to user | `copy_to_user` | null status pointer は許可します。non-null status pointer は writable な 32-bit wait status word を指す必要があります。blocking wait は sleep 前に pointer を validate し、waiting parent の address space へ戻った後に status を書きます。 |
...
- `execve` validation の valid `argv` / `envp`、bad pointer array、argument-count overflow、
  missing path、directory target、non-ELF file、no-return successful self-`execve`。
- userland spawn smoke の explicit `argv` / `envp` vector と spawned child image 内の validation。
- userland wait smoke の pending `WNOHANG`、blocking `WAIT_ANY`、waiting parent address space 経由の
  nonzero status storage。
...
- `UserCString` は terminating NUL byte が見つかるまで candidate readable range を表します。
  まだ完全に validated された C-string type ではありません。
