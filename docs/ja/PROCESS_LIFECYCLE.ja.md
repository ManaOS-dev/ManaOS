...
現在の shell は stdin を EOF または `exit` まで固定バッファへ読み続け、transient な
try-again read を retry し、heap-free whitespace tokenization を
検証し、fixed-buffer `argv` を構築し、`/disk/bin/file_demo --shell-command-smoke` と
...
historical shell-loop EOF path も検証します。storage smoke で起動される shell は standard input として
`/dev/keyboard` を継承し、keyboard stdin queue が空の間は EOF ではなく read wait で block し、
`exit\n` を queue した後に wake して keyboard-backed exit path を検証します。
...
scheduler lifecycle invariant:

- Active: active user task は retained user task を参照し、finished でも address-space reclaiming 中でもなく、
  schedulable address space を所有している必要があります。active な blocked task は waiting であり得ますが、
  wake path が `Ready` へ戻すまでは runnable ではありません。
- Waiting: blocked user task は retained exit status を持たず、active user set に残り、sleep、`waitpid`、
  keyboard-backed `read` の完了後に saved syscall/timer state を resume できるよう user address space を保持します。
- Zombie: waitable exit status を持つ finished user task は active user set に残ってはいけません。parent collection は
  scheduler metadata と child exit record を使うため、address space と scheduler kernel stack はすでに reclaim 済みでも
  かまいません。
- Reaped: exit status が collection 済みの finished user task は retained diagnostic row としてだけ残ります。
  active ではなく、final smoke path では user address space と scheduler kernel stack が reclaim 済みであることを期待します。
...
- storage smoke は active-set drain、waiting transition の実行、残存 zombie が 0 であること、
  final reaped task count を確認した後に `Scheduler lifecycle invariants verified` を記録します。
...
- storage smoke は experimental `user_shell` ELF が disk image に存在し、`/disk/bin/user_shell` として
  ...
  `bin/missing_shell_command` の bounded error message を検証し、command loop に入った後で空の
  keyboard-backed stdin により一度 block し、`exit\n` が queue されると wake して
  initial process 経由で collect されることを検証します。その直後に storage smoke は
  non-interactive console smoke path で kernel console の `pwd` command を実行し、experimental shell が
  kernel console command availability を奪っていないことも証明します。
