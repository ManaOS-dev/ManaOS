...
現在の shell は stdin を固定バッファへ1回読み、heap-free whitespace tokenization を検証し、standard input が
まだ `/dev/null` なので EOF を検出して正常終了します。keyboard-backed stdin で interactive lifetime を
持たせる作業は未完了です。
...
- storage smoke は experimental `user_shell` ELF が disk image に存在し、`/disk/bin/user_shell` として
  登録され、lifecycle gate 後に起動され、whitespace tokenization を検証し、stdin EOF 後に initial process
  経由で collect されることを検証します。
