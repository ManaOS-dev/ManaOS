...
failure を stable な errno-facing result に分類します。最小 user-visible `spawn` syscall と no-std
wrapper は、smoke と shell bring-up 向けに path-only child launch surface を公開します。
...
surface は、その directory で executable path を1つ解決して child を即座に active set へ入れます。
blocking `waitpid`、argv/envp 付き spawn、完全な descriptor inheritance policy は今後の作業です。
