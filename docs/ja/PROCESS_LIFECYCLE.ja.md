...
close-on-exec descriptor metadata、user-visible child creation、scheduler-backed な `waitpid` の
wait/reap state machine は今後の作業です。`waitpid` の syscall number、option constant、
no-std userland wrapper は予約済みなので、後続の child-exit 実装は安定した ABI target に
向けて進められます。
...
現在の kernel dispatch は、scheduler-owned child exit record が実装されるまで `waitpid` に
`-ENOSYS` を返します。storage smoke は no-std userland wrapper 経由でこの unsupported result を
検証しているため、後続の behavior change は明示的に変わります。

将来の scheduler-backed contract:
...
- caller に matching child process が存在しない場合は `-ECHILD` を返します。
- ManaOS に user interrupt policy が文書化されるまで、`-EINTR` は返しません。
...
close-on-exec flag はまだ存在しないため、次の descriptor metadata step では既存の descriptor
number と offset を default で保持し、明示的に close-on-exec と mark された descriptor だけを閉じます。
