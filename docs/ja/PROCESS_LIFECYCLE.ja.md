...
wrapper は、この task-owned directory を no-std userland code に公開します。
user-visible child creation と scheduler-backed な `waitpid` の wait/reap state machine は今後の作業です。
