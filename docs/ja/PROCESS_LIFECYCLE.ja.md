...
spawn helper は、将来 user-visible な spawn syscall を追加する前段階として、executable path lookup
failure と image-buffer allocation failure を stable な errno-facing result に分類します。
...
task creation 時点の parent current working directory をコピーします。user-visible child creation と
scheduler-backed な `waitpid` の wait/reap state machine は今後の作業です。
...
継承した process metadata、新しく構築した address space から child task を直接作る方針です。
...
non-child selector path を検証し、scheduler-owned selected-child reap path も検証するため、後続の
