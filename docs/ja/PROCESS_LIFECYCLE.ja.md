...
現在の shell は stdin を EOF または `exit` まで固定バッファへ読み続け、heap-free whitespace tokenization を
...
- storage smoke は experimental `user_shell` ELF が disk image に存在し、`/disk/bin/user_shell` として
  ...
  `bin/missing_shell_command` の bounded error message を検証し、command loop に入った後で stdin EOF に到達し、
