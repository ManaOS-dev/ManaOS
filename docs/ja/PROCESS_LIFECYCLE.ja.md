...
shared syscall number と no-std userland wrapper は予約済みです。kernel runtime implementation は
まだ未実装なので、lifecycle replacement path が追加されるまでは `SYS_EXECVE` は unsupported syscall
result を返します。
...
- `argv == NULL` を empty argument vector として扱う場合は、その compatibility choice を
  syscall policy table に明記します。
