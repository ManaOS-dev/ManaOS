...
empty input、token overflow、argument-buffer exhaustion、`bin/missing_shell_command` の bounded command
error message も検証します。standard input はまだ `/dev/null` なので EOF を検出して正常終了します。keyboard-backed stdin で interactive lifetime を
持たせる作業は未完了です。
...
- storage smoke は experimental `user_shell` ELF が disk image に存在し、`/disk/bin/user_shell` として
  ...
  path execution で起動して wait し、empty command、token-limit overflow、argument-buffer exhaustion、
