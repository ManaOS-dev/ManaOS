...
- ACPI RSDP、root table、MADT、Local APIC、IOAPIC の physical address は、
  boot logging または APIC routing setup が final formatting / architecture-MMIO
  boundary を越えるまで parser diagnostics 内で `PhysAddr` として保持します。
...
- Local APIC timer の calibration / active status diagnostics は、final serial-log formatting
  boundary まで timer MMIO base を `ApicMmioAddress` として保持します。
...
- ACPI physical address diagnostics が serial-log formatting 直前まで
  `PhysAddr` として保持されること。
...
- periodic Local APIC timer tick が scheduler progress を進め、timer MMIO base が
  diagnostic output 直前まで typed のまま保持されること。
