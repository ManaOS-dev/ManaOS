...
- `paging::map_kernel_mmio_range(...)` の MMIO physical base `PhysAddr`。
...
- `PageCount`: virtual range reservation、user stack allocation、private user mapping tracking 前の non-zero 4 KiB page count。
...
- `UserMappings` は private mapping record count を `PageCount` として保持し、`map_private(...)` / `unmap_range(...)` は scheduler diagnostics の aggregate counter に畳み込む前に typed page count を返します。
...
- `PageCount`: kernel virtual range allocator API、user stack API、user mapping API に渡す non-zero 4 KiB page count。
