...
- `PhysicalFrameStart` / `PhysicalFrameRange`: allocatable 4 KiB frame start と contiguous frame ownership。
...
- `PhysicalFrameAllocator::add_region(...)` と `reserve_region*` の physical start。
...
- `PhysicalFrameRange`: frame start と page count。
