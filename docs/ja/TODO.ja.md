...
spawn descriptor inheritance selection は文書化済みで、storage smoke で snapshot diagnostic を
確認します。child-private な enforcement は descriptor table を process-owned metadata へ移した後に
残っています。parent-exit-while-child-lives smoke は追加済みで、現在は original parent に retained
された exit record を diagnostics で確認します。残りは descriptor inheritance enforcement、
initial-process reparenting policy、reaping state です。
...
1. orphaned child 向けの initial-process reparenting policy を定義して実装する。
2. process-owned descriptor table 化で spawn descriptor inheritance enforcement を入れる。
...
5. user shell は runtime と process lifecycle が安定してから initial interactive process 化へ進む。
