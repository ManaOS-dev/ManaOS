...
reclaim、no-return self-`execve` smoke、open descriptor inheritance smoke は
[`PROCESS_LIFECYCLE.ja.md`](PROCESS_LIFECYCLE.ja.md) に整理済みです。次の `execve` 周辺作業では、
current directory、close-on-exec metadata / close behavior、replacement-state diagnostics、
second program smoke を小さい branch に分けて進めます。
...
1. `execve` の残りとして current directory、close-on-exec、replacement-state diagnostics、
