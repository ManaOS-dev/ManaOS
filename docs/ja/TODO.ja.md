...
absolute path execution、relative path execution、`file_demo` launch smoke、EOF 終了 smoke は完了済みです。
残りは keyboard-backed stdin で動き続ける interactive lifetime、built-in command、error message です。
...
waitable exit が残ることまで確認します。残りは smoke-started user shell の keyboard-backed interactive
lifetime と、general user process lifecycle 側の preemption/scheduler diagnostics です。
...
1. smoke-started userland shell を keyboard-backed stdin で動き続けるようにする。
2. command failure 用の bounded error message を追加する。
3. missing command の shell smoke log を追加する。
4. general spawned user process lifecycle へ timer preemption を拡張する。
5. lifecycle state が増えたら scheduler diagnostics も同期する。
6. address-space cleanup と per-task kernel stack cleanup の smoke coverage を広げる。
7. shell command 実装は fixed buffer と no-std runtime 制約を守って小さく分ける。
