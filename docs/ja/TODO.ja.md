...
`execve` の kernel-side contract、shared syscall number、no-std userland wrapper、cleanup invariant は
[`PROCESS_LIFECYCLE.ja.md`](PROCESS_LIFECYCLE.ja.md) に整理済みです。次の実装では、
argv/envp copy-in、failure rollback code、smoke coverage を
小さい branch に分けて進めます。
...
1. 文書化済みの `execve` contract に沿って、argv/envp copy-in と failure rollback code を追加する。
