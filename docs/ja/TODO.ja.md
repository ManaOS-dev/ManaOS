...
post-shell kernel console availability smoke も完了済みです。残りは QEMU 上で
experimental user shell に入って抜ける手順の documentation です。
...
waitable exit が残ることまで確認します。残りは experimental user shell の QEMU 操作手順と、
general user process lifecycle 側の preemption/scheduler diagnostics です。
