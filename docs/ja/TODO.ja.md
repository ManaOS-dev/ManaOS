...
`execve` の kernel-side contract と cleanup invariant は
[`PROCESS_LIFECYCLE.ja.md`](PROCESS_LIFECYCLE.ja.md) に整理済みです。次の実装では、
shared syscall number、userland wrapper、argv/envp copy-in、failure rollback code、smoke coverage を
小さい branch に分けて進めます。
...
1. 文書化済みの `execve` contract に沿って、shared syscall number と userland wrapper を追加する。
