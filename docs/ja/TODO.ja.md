...
bounded staging、filesystem path validation、cleanup invariant は
[`PROCESS_LIFECYCLE.ja.md`](PROCESS_LIFECYCLE.ja.md) に整理済みです。次の実装では、
successful `execve` sentinel、scheduler-owned publish、old image reclaim、no-return smoke
を小さい branch に分けて進めます。
...
1. 文書化済みの `execve` contract に沿って、successful publish path と no-return smoke を追加する。
