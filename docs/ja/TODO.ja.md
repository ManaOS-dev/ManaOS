...
実行されます。fixed-buffer stdin read、heap-free whitespace tokenization、EOF 終了 smoke は完了済みです。
残りは keyboard-backed stdin で動き続ける interactive lifetime と command 実装です。
