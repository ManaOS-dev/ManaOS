...
scheduler resume handoff diagnostic snapshot、user virtual-memory scheduler snapshot は typed 境界を通るようになっているため、次の typed address
...
2. 残っている memory API の狭い範囲で raw virtual address parameter を `VirtAddr` に置き換える。kernel stack guard-fault address は typed boundary 化済みです。
