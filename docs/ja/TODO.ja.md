...
EOF 終了 smoke は完了済みです。残りは keyboard-backed stdin で動き続ける interactive lifetime、
built-in command、broader command failure messages です。
...
4. lifecycle state が増えたら scheduler diagnostics も同期する。
5. address-space cleanup と per-task kernel stack cleanup の smoke coverage を広げる。
6. shell command 実装は fixed buffer と no-std runtime 制約を守って小さく分ける。
