...
reclaim、no-return self-`execve` smoke は [`PROCESS_LIFECYCLE.ja.md`](PROCESS_LIFECYCLE.ja.md) に
整理済みです。次の `execve` 周辺作業では、current directory / descriptor inheritance、close-on-exec、
replacement-state diagnostics、second program smoke を小さい branch に分けて進めます。
...
1. `execve` の残りとして current directory / descriptor inheritance、close-on-exec、replacement-state diagnostics、
   second program smoke のどれかを小さく切る。
