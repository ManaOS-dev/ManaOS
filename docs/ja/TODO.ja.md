...
- 最小 user shell の導入。
- syscall return frame と timer interrupt frame を one scheduler path で扱うこと。
...
waitable exit が残ることまで確認します。resumed user process の full runtime trap-frame restore は
scheduler diagnostics と storage smoke で確認済みです。残りは general user process lifecycle 側の
syscall/timer return path consolidation と preemption/scheduler diagnostics です。
...
1. syscall return frame と timer interrupt frame を one scheduler path で扱う。
2. resumed user task ごとに個別の address-space root と kernel stack が使われることを確認する。
