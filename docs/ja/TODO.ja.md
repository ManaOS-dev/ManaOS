...
`brk` user heap request、`munmap` user mapping request、user task kernel stack top handoff、
scheduler-owned kernel stack guard / writable start、scheduler resume handoff diagnostic snapshot、
...
2. 残っている memory API の狭い範囲で raw virtual address parameter を `VirtAddr` に置き換える。kernel stack guard-fault address と scheduler-owned guard / writable start は typed boundary 化済みです。
