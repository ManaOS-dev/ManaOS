...
- 最小 user shell の導入。
- resumed user task ごとに個別の address-space root と kernel stack が使われること。
...
で確認済みです。残りは general user process lifecycle 側の address-space/kernel-stack resume 証明と
...
1. resumed user task ごとに個別の address-space root と kernel stack が使われることを確認する。
2. lifecycle state が増えたら scheduler diagnostics も同期する。
3. address-space cleanup と per-task kernel stack cleanup の smoke coverage を広げる。
