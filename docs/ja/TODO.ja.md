...
- 最小 user shell の導入。
- resumed user process の trap frame 復元を一般化すること。
...
1. resumed user process ごとに full runtime user trap frame を復元する。
2. syscall return frame と timer interrupt frame を one scheduler path で扱う。
