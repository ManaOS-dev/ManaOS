...
- `spawn + execve` を最初の安定モデルとして進めるための実装。
- 最小 user shell の導入。
...
storage smoke で確認済みです。address-space reclaim 中 task の scheduling prevention、
active、finished、reclaiming transition の impossible state assertion、active、waiting、zombie、reaped task の
invariant documentation、storage smoke で走る scheduler invariant check も確認済みです。
次は Phase 2 の typed address boundary を小さい branch に分けて進めます。

## Phase 2: Memory Safety, Address Spaces, And Stack Hardening

このフェーズは、物理/仮想アドレス、user mapping、kernel stack、`mmap` 周辺の
安全性を高める作業です。
`PhysicalFrameStart` construction はすでに `PhysAddr` 必須、`UserVirtualAddress` construction は
`VirtAddr` 必須、user mapping/unmapping は `UserPageStart` 必須、frame allocator API は
`FrameCount` 必須、kernel virtual range allocator API、user stack API、user mapping API、paging helper API は `PageCount` 必須になっているため、
`brk` user heap request、`munmap` user mapping request、dynamic kernel virtual range start、
user task kernel stack top handoff、scheduler-owned kernel stack guard / writable start、
scheduler resume handoff diagnostic snapshot、
user virtual-memory scheduler snapshot は typed 境界を通るようになっているため、次の typed address
boundary 作業はそれ以外に残る raw physical / virtual address 境界と alignment-required address 境界から選びます。
...
2. 残っている memory API の狭い範囲で raw virtual address parameter を `VirtAddr` に置き換える。dynamic kernel virtual range start、kernel stack guard-fault address、scheduler-owned guard / writable start は typed boundary 化済みです。
