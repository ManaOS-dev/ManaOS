...
- 最小 user shell の導入。
- preempted/resumed user process の trap frame 保存と復元を一般化すること。
...
waitable exit が残ることまで確認します。残りは general user process lifecycle 側の
...
1. preempted user process ごとに full runtime user trap frame を保存する。
2. resumed user process ごとに full runtime user trap frame を復元する。
