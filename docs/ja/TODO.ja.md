...
scheduler resume handoff diagnostic snapshot は typed 境界を通るようになっているため、次の typed address
boundary 作業はそれ以外に残る raw physical / virtual address 境界、scheduler snapshot 内の残りの address 値、
alignment-required address 境界から選びます。
