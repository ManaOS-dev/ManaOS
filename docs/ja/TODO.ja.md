...
userland child wait smoke、nonzero child exit status smoke まで完了済みです。
...
残りは replacement-state diagnostics、blocking `waitpid`、descriptor inheritance policy、
reaping state です。
...
1. `execve` の残りとして replacement-state diagnostics を小さく切る。
