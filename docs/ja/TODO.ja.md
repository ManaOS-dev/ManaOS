...
`cd` / `help` / `pwd` built-in smoke、bounded command error message smoke、EOF 終了 smoke は完了済みです。
残りは keyboard-backed stdin で動き続ける interactive lifetime と `exit` built-in command です。
