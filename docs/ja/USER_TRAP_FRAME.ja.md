...
保存済み `UserTrapFrame` の byte size、最後に復元した frame の byte size、保存済み runtime frame からの
restore 回数を公開します。storage smoke は、timer-preempted user task snapshot すべてに recorded
interrupt frame、full frame size、runtime-frame restore があることを assert します。
...
- user task metadata が saved trap frame を所有し、task lifecycle helper 経由でだけ公開する。
  scheduler snapshot は recorded frame flag、full saved frame byte size、full restored frame byte
  size、runtime-frame restore count を公開し、smoke assertion で確認します。
...
- `just storage-smoke` が one-shot user path、Local APIC timer preemption、resume、separate stack slot、
  separate address space、lifecycle diagnostics、full restored trap-frame size、timer-preempted snapshot
  の runtime trap-frame restore を証明する。
