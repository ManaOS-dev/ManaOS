...
user smoke task が unified scheduler record path を使うことと、timer-preempted user task snapshot
すべてに recorded interrupt frame、full frame size、runtime-frame restore があることを assert します。
...
user selector を埋め、`UserTrapFrameSource::Syscall` 付きの
...
- user task metadata が saved trap frame を所有し、task lifecycle helper 経由でだけ公開する。
  scheduler snapshot は recorded frame flag、full saved frame byte size、unified runtime-frame record
  count、full restored frame byte size、runtime-frame restore count を公開し、smoke assertion で確認します。
- per-task kernel stack が存在し、user task entry/resume 前に install される。scheduler snapshot は
  last resume handoff count、selected address-space root、selected kernel stack top を保持し、
  smoke assertion で確認します。
...
- `tasks` console command が retained task ごとに lifecycle state、active membership、address-space ownership、
  scheduler-managed kernel stack ownership、last user resume address-space/kernel-stack handoff を表示する。
...
- `just storage-smoke` が one-shot user path、Local APIC timer preemption、resume、separate stack slot、
  separate address space、lifecycle diagnostics、unified trap-frame record path、full restored
  trap-frame size、timer-preempted snapshot の runtime trap-frame restore、各 finished user task の
  nonzero resume handoff count / address-space root / kernel stack top を証明する。
