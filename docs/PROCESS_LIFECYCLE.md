...
verifies bounded command error messages for empty input, token overflow,
...
- Storage smoke verifies that the experimental `user_shell` ELF is present in
  ...
  execution, waits for both children, verifies bounded error messages for an
  empty command, token-limit overflow, argument-buffer exhaustion, and
