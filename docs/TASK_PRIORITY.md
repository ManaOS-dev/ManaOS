...
the userland runtime path API, compiled-in `help` output, plus bounded command
error smoke for empty input, token overflow, argument-buffer exhaustion, and
missing commands, are documented in
