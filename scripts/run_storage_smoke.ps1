...
$expectedPatterns = @(
    ...
    "execve image published -> task=.* path=/disk/bin/smoke_demo .*argc=2 old_user_pages=12 old_page_table_pages=10 trap_frame_user_addresses_typed=true",
    "close-on-exec descriptors closed: count=[1-9][0-9]*",
    "execve image published -> task=.* path=/disk/bin/file_demo .*argc=1 old_user_pages=10 old_page_table_pages=7 trap_frame_user_addresses_typed=true",
