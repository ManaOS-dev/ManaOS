...
$expectedPatterns = @(
    ...
    "Scheduler diagnostics verified: .*user_tasks=2 .*finished=2 .*active_user_address_spaces=0 .*pending_user_exits=0 .*reclaimed_user_kernel_stacks=2 .*reclaimed_kernel_stack_writable_pages=8 .*reclaimed_kernel_stack_virtual_pages=10 .*timer_preemptions=.* user_resumes=.* finished_tasks=2",
