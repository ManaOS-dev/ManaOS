...
- [x] Keep private `munmap` syscall lengths typed as `UserMappingLength` before unmap helpers derive page counts, with storage smoke coverage.
...
- [x] Keep ELF segment heap-start align-up typed as `UserVirtualAddress::align_up_to_page()`, with storage smoke coverage.
...
- [x] Keep fixed private mapping placement end validation typed through `UserPageStart`, with storage smoke coverage.
