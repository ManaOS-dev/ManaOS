...
fn main() {
    ...
    let file_demo = extract_binary(&manifest_dir, &target_dir, &profile, "file_demo");
    let bad_pointer_demo = extract_binary(&manifest_dir, &target_dir, &profile, "bad_pointer_demo");
...
fn extract_binary(
    ...
    let elf_path = target_dir
        ...
        .join(profile_dir)
        .join(binary_name);
    let binary_dir = target_dir.join("bin");
