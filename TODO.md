...
- [ ] Save and restore a full user trap frame on interrupt and syscall paths; one-shot user entry restores an initial full trap frame, and SYSCALL entry now switches to the task kernel stack before capturing runtime user frames. Interrupt capture remains.
...
- [ ] Add kernel stack switching per task where needed; user task stacks are installed before entry and SYSCALL switches onto them, but interrupt entries still need the same treatment.
